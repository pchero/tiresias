...
bool db_ctx_backup(db_ctx_t* ctx, const char *filename)
{
  ...
  if(backup == NULL) {
  	slog(LOG_WARNING, "Could not initiate backup database.");
  	return false;
  }
  ...
  while(1) {
  	...
  	if((ret != SQLITE_OK) && (ret != SQLITE_BUSY) && (ret != SQLITE_LOCKED)) {
  		slog(LOG_ERR, "Could not backup the database. ret[%d]", ret);
  		break;
  	}
...
bool db_ctx_load_db_schema(db_ctx_t* ctx, const char* filename)
{
  ...
  if(ret != SQLITE_OK) {
  	slog(LOG_ERR, "Could not open the database.");
  	return false;
  }
...
bool db_ctx_load_db_all(db_ctx_t* ctx, const char* filename)
{
  ...
  if(ret == false) {
    slog(LOG_ERR, "Could not load db scema.");
    return false;
  }
